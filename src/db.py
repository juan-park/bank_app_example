...
def add_user(username: str, first_name: str, last_name: str, birthday: str, datesignedup: str, phonenumber: str, email: str, address: str, password: str, balance: str):
    """FYI the date format used is dd/mm/yyyy. Feel free to try changing it.
    """
    sql = f"""INSERT INTO `{DB_NAME}`.`user` (`username`, `first name`, `last name`, `birthday`, `datesignedup`, `phonenumber`, `email`, `address`, `password`, `balance`) VALUES ('{username}', '{first_name}', '{last_name}', STR_TO_DATE('{birthday}', '%d/%m/%Y'), STR_TO_DATE('{datesignedup}', '%d/%m/%Y'), '{phonenumber}', '{email}', '{address}', '{password}', {balance});"""
    cur = conn.cursor()
    cur.execute(sql)
    conn.commit()
...
def login_sql(username: str, password: str) -> int:
    pass
